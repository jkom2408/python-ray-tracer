...
Also, it is able to map these points to an in-memory canvas object, and then save the canvas object to disk as a .ppm file (the initial values used to generate the image are different from the inital values used to generate the coordinates above):
